...
export default function CartDrawer() {
  ...
  const { items, dispatch, drawerOpen, closeDrawer, openCheckout, totalItems, totalPrice } = useCart()
  const currency = items[0]?.currency ?? 'DA'

  return (
    <>
      <div className={`cart-backdrop${drawerOpen ? ' open' : ''}`} onClick={closeDrawer} aria-hidden="true" />

      <aside className={`cart-drawer${drawerOpen ? ' open' : ''}`} role="dialog" aria-modal="true" aria-label={t('cart.title')}>
        ...
        {items.length > 0 && (
          <div className="cart-drawer__footer">
            ...
            <button className="cart-drawer__checkout-btn" onClick={openCheckout}>
            ...
            <p className="cart-drawer__secure">🔒 100% Secure Payments</p>
          </div>
        )}
